...
这个目录用纯 Python 标准库拆解现代生成式 AI 中常见的四个基础概念。案例不会下载模型、不会调用在线 API，也不追求生产性能，重点是让初学者看清数据如何一步一步变化。
...
| 2 | [迷你 BPE Tokenizer](examples/mini_bpe_tokenizer.py) | 子词、词频、合并规则、编码 | 标准库 |
| 3 | [迷你 RAG 检索流程](examples/mini_rag_retrieval.py) | 本地知识库、TF-IDF、余弦相似度、上下文拼装 | 标准库 |
| 4 | [LoRA 低秩更新](examples/lora_low_rank_update.py) | 冻结原权重、低秩矩阵、参数量对比 | 标准库 |
