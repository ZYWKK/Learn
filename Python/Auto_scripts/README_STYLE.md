...
- README 可以保留关键片段，但完整可运行代码应以同目录 `.py` 文件为唯一来源。
- 如果为了保留早期教学内容而展示旧版代码，要在标题后明确链接到 [统一运行与安全说明](STANDARDIZED_USAGE.md)。
- 修改函数参数、默认行为、依赖或环境变量后，必须同步修改使用方式和风险提醒。
- 删除、移动、覆盖类脚本应说明默认是否预演，以及如何明确开启实际执行。
